...
const buildMenuGroupsFromRecords = (records = []) => {
  ...
  records
    .filter((item) => item && item.is_active !== false && groups[item.section_key])
    .sort((a, b) => {
...
const renderFeaturedMenu = () => {
  ...
  const featuredItems = (currentMenuGroups.showcase?.items || []).slice(0, 4);
...
const renderExpandedMenu = (category) => {
  const menuGroup = currentMenuGroups[category] || currentMenuGroups.showcase || fullMenuItems.showcase;
...
menuFilters.forEach((filter) => {
  filter.addEventListener("click", () => {
    activeMenuCategory = filter.dataset.category || "showcase";
    renderExpandedMenu(activeMenuCategory);
  });
});
